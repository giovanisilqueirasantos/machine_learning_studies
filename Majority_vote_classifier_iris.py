...
from utils.MajorityVoteClassifier import MajorityVoteClassifier
import numpy as np
...
clf_labels = ['Logistic Regression', 'Decision Tree', 'KNN', 'Majority Votring']
...
for clf, label, ls in zip(all_clf, clf_labels, linestyles):
    ...
    plt.plot(fpr, tpr, linestyle=ls, label=f'{label} (auc = {roc_auc:.2f})')
